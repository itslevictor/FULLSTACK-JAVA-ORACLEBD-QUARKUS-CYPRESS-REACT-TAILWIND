describe('Inventory Management System E2E', () => {
  // sufixo aleatório para evitar que o banco de dados 
  ...
  beforeEach(() => {
    // Resolução para que os elementos 'lg:block' apareçam
  ...
  it('should register a raw material', () => {
    ...
    // o primeiro input de número (estoque)
    cy.get('input[type="number"]').first().clear().type('100');
    
    // especificamente no botão de adicionar material (o primeiro "Add" da página)
  ...
  it('should calculate production suggestion correctly', () => {
    ...
    // espera o material aparecer na lista lateral/dashboard antes de prosseguir
    ...
    // o nome único, ele será o último ou um dos últimos da lista
    cy.get('select').select(`${materialName} (Stock: 10)`);

    // Quantidade necessária para o produto (é o input com placeholder "0" dentro do BOM)
    // ele costuma ser o último input de número renderizado
    cy.get('input[placeholder="0"]').last().clear().type('2');
    

    // Em vez de 'cy.get(button).contains(Add)', ancorei no container da receita (BOM)
    // para garantir que se clica no botão "Add" do formulário de PRODUTO.
    ...
    // O sistema chama o Backend Java.
    cy.get('h2').contains('Production Suggestion').scrollIntoView();
    
    // timeout para 10s para dar tempo do Java processar e o Redux atualizar a tela
    ...
    // Procura especificamente dentro do card do produto recém criado
